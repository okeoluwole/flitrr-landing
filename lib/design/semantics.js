...
// ── The STACK appraisal verdict: GO, CONSIDER, NO GO ──
// Vocabulary: lib/stack/engine/verdict.js (decisionBand, the workbook's
// Summary A19), spent by the report's decision tag and the sensitivity
// grid's cell reads. The verdict is a computed judgement of the whole
// scheme against its target, so it takes the objective ladder's argument
// rather than inventing a fourth colour story: colour spends nothing on a
// scheme that clears its hurdle (GO reads as calm strength, ink on the
// strong outline, and never the recorded-fact green, because a verdict is
// exactly what the green rule excludes), CONSIDER wears the ladder's
// bright exposure amber (inside the consider band: exposed, not failed),
// and NO GO is the one filled read, in breach red, because below the band
// is the appraisal's breach state. Shape: the 4px tag, severity's box,
// since a verdict is a derived read, never a classification pill.
const STACK_VERDICT_APPEARANCE = Object.freeze({
  GO: Object.freeze({
    label: 'Go',
    ink: '--app-ink',
    fill: null,
...
/**
 * The parts of an appearance that change register when it crosses to paper.
 *
 * markFill was missing from this list until it was noticed that a paper
 * appearance could still hand back an --app-* mark colour: the glyph would
 * keep its screen ink while the label around it moved to --doc-*. Nothing
 * hit it, because the only marked appearance the Brief renders is the
 * criticality mark, whose colour travels as `fill`. That is why it never
 * bit, not why it was safe.
 *
 * Every other field is medium-independent by construction. A label is a
 * word, a weight is a number, and borderStyle, shape and mark are carriers
 * whose whole point is that they survive losing colour: they read the same
 * on a screen, on paper and to a reader who sees neither.
 */
...
export function onPaper(appearance) {
  const crossed = { ...appearance };
  // Shape-preserving: an appearance with no mark has no markFill key, and
  // its paper counterpart should not acquire one. onPaper changes a
  // value's medium, never the fields it has.
  for (const part of CROSSING) {
    if (part in appearance) crossed[part] = paper(appearance[part]);
  }
...
/** STACK appraisal verdict: the engine's own 'GO' | 'CONSIDER' | 'NO GO'
 *  (decisionBand), keyed unmapped so the surface hands the decision straight
 *  through and no translation layer can drift. */
export function stackVerdictAppearance(decision) {
  return resolve(STACK_VERDICT_APPEARANCE, 'STACK verdict', decision);
}
...
export const SEMANTIC_MAPPINGS = Object.freeze([
  ...
  Object.freeze({
    id: 'stack-verdict',
    name: 'STACK appraisal verdict',
    note: "The appraisal's decision read, on the ladder's argument: calm spends nothing on a scheme that clears its hurdle, the consider band is exposure amber, and below the band is the one filled read, in breach red.",
    values: Object.freeze(['GO', 'CONSIDER', 'NO GO']),
    appearance: stackVerdictAppearance,
  }),
]);
