...
const OPENING_LINE =
  'The critical actions open across your projects, grouped by project.';
...
export function buildDigestHtml(digest, unsubscribeUrl, appUrl) {
  const projectBlocks = digest.projects
    .map((project) => {
      ...
      return `
        <tr><td style="padding: 18px 0 4px; font-size: 13px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: ${GREY_BLUE};">${escapeHtml(project.name)}</td></tr>
        ${items}`;
    })
    .join('');
