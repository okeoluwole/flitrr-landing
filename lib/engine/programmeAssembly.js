 ...
 * two-level programme: every gate and every milestone dated, each milestone
 * carrying its served objective and its baked criticality, each activity carrying
 * its duration. This is the content that becomes v1 at lock.
 *
 ...
 *
 *  3. Place every milestone.
 *     - A headline milestone, which the developer dated, takes its agreed date.
 *       That is authoritative even where it differs from a start-plus-offset
 *       position, because the developer set it (origin 'carried').
 *     - A drill-down milestone, which the developer never dated, takes a baseline
 *       date derived from its offsetWeeks on the agreed skeleton, placed at the
 *       agreed stage start plus the absolute offset (origin 'added'). The offset is
 *       not scaled to fit a moved gate. Where an offset lands past its agreed gate
 *       it is left as it falls, an honest first-draft position parked for the
 *       durations pass, not smoothed here.
 *
...
/**
 * Place one template milestone on the agreed skeleton.
 *
 ...
 * never flagged). A milestone the developer never dated is a drill-down, placed
 * at the agreed stage start plus its absolute curated offset and tagged added.
 */
function placeMilestone(milestone, stageStartEpoch, choice, byType, resolutionByKey) {
  ...
  if (resolvedEpoch != null) {
    ...
    // Drill-down: placed by the absolute offset from the agreed stage start, never
    // scaled to fit a moved gate, and never clamped where it lands past its gate.
  ...
  return {
    ...
    criticality: classifyByType(milestone.serves, byType),
 ...
 *
 * Returns the fully-resolved programme, every date computed and nothing left to
 * re-derive, the snapshot-ready content 2.2 freezes as v1:
 *   {
