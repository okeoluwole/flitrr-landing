...
import { STAGE_STATE } from './stageStates.js';
...
function placeMilestone(
  ...
  if (resolvedEpoch != null) {
    ...
    // point that serves a protected objective. In a stage completed prior to
    // adoption nothing is derived at all: its dates are the developer's
    // backfilled record of what happened, and the engine invents no history.
...
// A stage completed prior to adoption holds no stage start either, but its
// milestones ARE assembled, carrying the developer's backfilled dates where
// recorded and staying honestly undated where not; nothing is derived in it.
function buildActivity(
  ...
  const milestones =
    stageStartEpoch == null && !completedPriorToAdoption
      ? []
      : milestonesDef.map((m) =>
          placeMilestone(
            m,
 ...
 * options       optional. options.stageStates is the stage states
 *               (deriveStageStates' output, or a per-stage array). Omitted,
 *               every stage reads as sequential and the assembly is unchanged.
 *               A concurrent stage places its milestones on its declared window
 *               anchor rather than on the previous gate, so a drill-down point
 *               in a stage 7 that runs alongside construction lands during the
 *               build instead of weeks after the completion gate. Its own gate
 *               is unaffected: a gate still closes its stage in sequence.
 *
 ...
 *         stageStart,        Date, or null when not applicable or when the
 *                            stage completed prior to adoption (its dates are
 *                            history, not a planned window)
 *         activities: [
 *           {
 *             key, name,
 ...
 *           key,                    `gate_<stage>`
 *           name,                   the stage name
 *           baselineDate,           Date, or null when not applicable; for a
 *                                   completed-prior-to-adoption gate this is
 *                                   the backfilled actual, or null when none
 *                                   is recorded yet, never a rolled fallback
 *           origin,                 'carried'
 *           completedPriorToAdoption, true when the gate closed before the
 *                                   project entered PULSE: recorded, never
 *                                   run through the gate ceremony
...
export function assembleProgramme(
  ...
  // stage start is the window start the chain returns: the previous applicable
  // gate's effective date for a sequential stage (the project start for the first
  // applicable one), and the declared window anchor for a concurrent stage. The
  // gate is this stage's own effective date, whatever its state. This is the same
  // anchor the Phase 1.1 reality check measured from, read from the same chain.
  const startByStage = new Map();
  const gateDateByStage = new Map();
  const stateByStage = new Map();
  let prevEffectiveEpoch = null;
  for (const rollingStage of rolling.stages) {
    stateByStage.set(rollingStage.stage, rollingStage.state);
    if (!rollingStage.applicable) continue;
    startByStage.set(
      rollingStage.stage,
      rollingStage.windowStart
        ? rollingStage.windowStart.getTime()
        : prevEffectiveEpoch == null
          ? startEpoch
          : prevEffectiveEpoch
    );
    // A complete stage's gate already happened before the project entered
    // PULSE: it carries the developer's backfilled date, and where none is
    // recorded yet it is honestly undated, never inherited from the previous
    // gate. A remaining undated gate keeps the rolled fallback.
    const isCompleteStage = rollingStage.state === STAGE_STATE.COMPLETE;
    ...
    // A concurrent stage does not push what follows it, matching the chain,
    // and an unbackfilled complete gate carries the anchor unchanged rather
    // than nulling it.
    if (rollingStage.state !== STAGE_STATE.CONCURRENT && effectiveEpoch != null) {
  ...
  const stages = stageDefs.map((stageDef) => {
    ...
    const closesActivityKey = activitiesDef.length
      ...
      : null;
    const stageState = stateByStage.get(stageDef.stage) ?? STAGE_STATE.SEQUENTIAL;
    // A complete stage finished before the project entered PULSE. Its dated
    // points are the developer's backfilled record of what happened, so the
    // engine derives nothing in it: a drill-down milestone with no backfilled
    // date stays honestly undated rather than taking an offset-derived date,
    // and its gate is recorded as completed prior to adoption, never run
    // through the gate ceremony.
    const completedPriorToAdoption = stageState === STAGE_STATE.COMPLETE;
    ...
    if (!applicable) {
      ...
      return {
        ...
        gate: {
          ...
          baselineDate: null,
          origin: ITEM_ORIGIN.CARRIED,
          completedPriorToAdoption: false,
          closesActivityKey,
        },
        locationSensitive: [],
      };
    ...
    return {
      ...
      applicable: true,
      state: stageState,
      // A complete stage has no planned window: its dates are history, so no
      // stage start is held and buildActivity places nothing by offset.
      stageStart: completedPriorToAdoption ? null : new Date(stageStartEpoch),
      activities: activitiesDef.map((a) =>
        buildActivity(
          a,
          completedPriorToAdoption ? null : stageStartEpoch,
          choice,
          byType,
          resolutionByKey,
          completedPriorToAdoption
