 ...
 *     It is set from the project's declared entry stage (note 12,
 *     baseline.entryStage, the Step 1 "Where is this project today?" answer):
 *     every stage before the declared stage reads as complete. A caller may
 *     also supply baseline.completedStages explicitly; the two combine.
 *
...
// The from-scratch entry stage, the Step 1 default. A project declared at it
// behaves exactly as a project set up before the field existed: nothing is
// complete, and every stage takes target-date decisions.
export const FROM_SCRATCH_ENTRY_STAGE = 1;

/**
 * The stages already finished before a project entered PULSE, from its
 * declared entry stage (note 12, mid-project adoption). Every stage strictly
 * before the declared stage is complete, with one deliberate exemption: at
 * the from-scratch entry stage (Stage 1, the default) nothing is complete,
 * so a project set up from scratch keeps Stage 0 as a datable part of its
 * plan, exactly as before the field existed. A missing or malformed entry
 * stage reads as from scratch.
 */
export function completedStagesForEntry(entryStage) {
  const entry = Number(entryStage);
  if (!Number.isInteger(entry) || entry <= FROM_SCRATCH_ENTRY_STAGE) return [];
  const completed = [];
  for (let s = 0; s < entry; s += 1) completed.push(s);
  return completed;
}
 ...
 *             entryStage            projects.entry_stage, the declared "Where
 *                                   is this project today?" answer from Step 1.
 *                                   Every stage before it reads as complete
 *                                   (completedStagesForEntry). Absent or at
 *                                   the from-scratch default, nothing is
 *                                   complete.
 *             completedStages       optional iterable of stage numbers already
 *                                   finished before the project entered PULSE,
 *                                   combined with the entry-stage derivation.
 *
...
export function deriveStageStates(template, baseline) {
  ...
  for (const s of completedStagesForEntry(baseline?.entryStage)) completed.add(s);
