 ...
 *         developer date minus the stage start (the previous applicable gate's
 *         effective date, the developer's date where set, otherwise its advised
 *         date; the project start for the first applicable stage). It is checked
...
// so a duration exactly on the generous band edge reads within_norm.
...
export function deriveRealityCheck(projectStart, template, choices, options) {
  ...
  const advised = deriveAdvisedDates(startEpoch, template, naStages);
  const advisedByStage = new Map(advised.stages.map((s) => [s.stage, s]));

  // The effective gate dates, for the spacing anchor. deriveRollingGateDates rolls
  // each gate from the previous applicable gate's effective date (the developer's
  // date where set, otherwise the rolled advised date), which is exactly the start
  // a stage's implied span and its milestones' implied offsets are measured from.
  // Using the rolled chain keeps the anchor continuous when a developer dates an
  // early gate away from advised, rather than jumping back to the absolute advised
  // date.
  const rolling = deriveRollingGateDates(startEpoch, template, choices);
  ...
  for (const rollingStage of rolling.stages) {
    ...
    // The first applicable stage starts at the project start; each later
    // applicable stage starts at the previous applicable gate's effective date.
    startByStage.set(
      rollingStage.stage,
      prevEffectiveEpoch == null ? startEpoch : prevEffectiveEpoch
    );
    prevEffectiveEpoch = rollingStage.effectiveDate
