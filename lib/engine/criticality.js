...
/**
 * The full capture stamp: the stored criticality plus the basis it derives
 * from, recorded together at the point of capture (Note 2). The basis is the
 * deriving objective and its classification at the moment of the write, so
 * "inherited from [objective]" is a read of recorded fact, not a re-join guess
 * at render time. An unlinked item stamps standard with a null basis, which is
 * the honest record that nothing conferred it.
 *
 * Returns { criticality, basisObjectiveId, basisType, basisClassification,
 * basisName }. basisName resolves only when the index was built with a
 * nameByType map (the kernel holds no display strings of its own).
 */
export function deriveCaptureStamp(linkedObjectiveId, byId) {
  const objective = linkedObjectiveId ? byId?.[linkedObjectiveId] : null;
  if (!objective) {
    return {
      criticality: CRITICALITY.STANDARD,
      basisObjectiveId: null,
      basisType: null,
      basisClassification: null,
      basisName: null,
    };
  }
  return {
    criticality:
      objective.classification === NON_NEGOTIABLE
        ? CRITICALITY.CRITICAL
        : CRITICALITY.STANDARD,
    basisObjectiveId: objective.id,
    basisType: objective.type,
    basisClassification: objective.classification ?? null,
    basisName: objective.name ?? null,
  };
}

/**
 * The write-path cap (Note 2, task 3): true when a proposed criticality value
 * exceeds what the linked objective carries, which is exactly one case:
 * critical proposed while the link does not derive critical (a flexible link,
 * no link, or a link that does not resolve). Nothing inherits more protection
 * than the objective it serves, so such a write must be rejected. The database
 * trigger (migration 034) mirrors this rule at the write path itself.
 */
export function exceedsLinkedObjective(value, linkedObjectiveId, byId) {
  return (
    value === CRITICALITY.CRITICAL &&
    toStoredCriticality(linkedObjectiveId, byId) !== CRITICALITY.CRITICAL
  );
}
