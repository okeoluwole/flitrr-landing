 ...
 *  - An unmet point's forecast is its rolled completion: the forecast, or
 *    actual, completion of what precedes it in the running order, plus its own
 *    baseline spacing, floored at today. The floor sits at the completion, not
 *    the start: a projection never lands in the past, but an unmet point holds
 *    its baseline-derived forecast until today passes it, and only then does it
 *    slip and cascade. That is the same observed-slip moment at which the RAG
 *    engine reads a point behind, so the colour and the forecast agree on which
 *    point is drifting. The accepted cost is that a badly overdue current point
 *    forecasts today, an understatement, rather than today plus a re-run of its
 *    whole span; the points after it still roll forward at their full spans
 *    from it. A met point's date may sit in the past; it is a fact.
 *  - The span of an unmet point is its baseline spacing: the signed gap from
 ...
 * its gate's forecast or actual. The programme forecast completion reads the
 * gates alone: the latest gate forecast or actual across the applicable stages,
 * the completion gate per the baseline. A milestone is never the programme
 * finish, so a derived or overhung tail point cannot drive the forecast
 * completion past the completion gate.
 *
...
function rollPoint(baselineEpoch, met, state, todayEpoch) {
  ...
  // A point the baseline holds undated (an undated headline point, or a
  // critical drill-down that takes no derived date) carries no baseline
  // segment. Its own forecast is still computed and shown, but it NEVER
  // re-anchors or advances the chain: converting its actual, or its floored
  // projection, into a drift would double-count spacing the baseline never
  // gave it, pushing phantom weeks into everything after it.
  const onChain = baselineEpoch != null;

  let forecastEpoch;
  if (met && met.metEpoch != null) {
    // A fact: the actual met date, re-anchoring the chain in either direction
    // where the point carries a baseline segment.
    forecastEpoch = met.metEpoch;
    if (onChain) state.positionEpoch = met.metEpoch;
  } else if (met) {
    ...
    // Unmet: the rolled completion, the predecessor's forecast or actual
    // completion plus this point's baseline spacing, floored at today. The
    // floor sits at the completion, not the start, so an unmet point holds its
    // baseline-derived forecast until today passes it and only then slips, the
    // same observed-slip moment at which the RAG engine reads a point behind.
    forecastEpoch = Math.max((state.positionEpoch ?? todayEpoch) + gap, todayEpoch);
    if (onChain) state.positionEpoch = forecastEpoch;
  }
 ...
 *   {
 *     forecastCompletion,      the programme finish: the latest gate forecast
 *                              or actual across the applicable stages (gates
 *                              only, never a milestone), respecting the
 *                              concurrency (whichever branch lands last), or
 *                              null when no gate is dated
 *     stages: [
 *       {
...
export function deriveForecast(baseline, metPoints, today) {
  ...
  // The programme finish reads the gates alone: a stage completes at its gate,
  // so the programme completes when the last applicable gate does, whichever
  // branch lands last (the concurrency is respected because every applicable
  // gate is a candidate). A milestone is never the finish: forecast completion
  // reads the completion gate per the baseline, so a derived or overhung point
  // sitting past its gate can never drive the completion beyond it.
  let finishEpoch = null;
  for (const node of stages) {
    if (node == null || node.applicable === false) continue;
    const candidate = node.gate?.forecastDate ?? null;
