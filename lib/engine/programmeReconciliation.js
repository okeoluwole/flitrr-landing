...
export const DERIVATION_RULES = Object.freeze({
  ...
  // A point in a stage completed prior to adoption with no backfilled actual
  // recorded yet. History the developer has not written down, not a plan the
  // engine declined to date.
  UNDATED_COMPLETED_STAGE: 'undated_completed_stage',
});
 ...
 * "23 Jul 2026". Never month and year alone. Null in, null out. The app's one
 * display format (dateFormat.js) under this module's name, rather than a second
 * implementation of the same rule.
 */
export const formatReconciliationDate = formatDisplayDate;
...
// epoch across the assembled programme. Gates only, never a milestone, and
// never a gate completed prior to adoption (Note 12): a backfilled historic
// date records what happened, it is not the plan's completion.
export function baselineCompletionGateEpoch(assembled) {
  ...
  for (const stage of assembled?.stages ?? []) {
    ...
    if (stage.gate?.completedPriorToAdoption === true) continue;
...
export function reconcileBaseline({
  ...
  for (const stage of assembled?.stages ?? []) {
    ...
    // A stage completed prior to adoption (Note 12). Its carried dates are the
    // developer's backfilled record of what happened and compare as recorded
    // fact exactly like any carried point; its template drill-downs are
    // honestly undated because the engine invents no history, and they are
    // disclosed under their own rule rather than the protected one.
    const completedPrior = stage.gate?.completedPriorToAdoption === true;
        ...
        if (milestone.origin === ITEM_ORIGIN.ADDED) {
          ...
          if (assembledEpoch == null) {
            derivations.push({
              ...
              baselineDate: null,
              rule: completedPrior
                ? DERIVATION_RULES.UNDATED_COMPLETED_STAGE
                : DERIVATION_RULES.UNDATED_PROTECTED,
