...
export function deriveAdvisedDates(projectStart, template, naStages, stageStates) {
  ...
  const stages = (template?.stages ?? []).map((stageDef) => {
    ...
    // A complete stage is history: it advises no gate and no milestones, and
    // its duration is not added, so the running gate date carries forward
    // unchanged to the first remaining stage, exactly as the rolling chain
    // carries its anchor. Its dates are the developer's backfilled record, not
    // a plan this derivation may draw.
    if (stageState.state === STAGE_STATE.COMPLETE) {
      return {
        stage: stageDef.stage,
        name: stageDef.name,
        applicable: true,
