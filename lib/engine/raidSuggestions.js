...
export function buildRaidBaseline({
  ...
  objectives = null,
  nameByType = null,
  // What the project has ALREADY captured, keyed by RAID type: the identity
  // text of each item currently in that list. A candidate whose rendered text
  // is already there is not offered again (see normaliseText).
  captured = null,
  // The candidate keys the project has already ANSWERED: the ones carrying a
  // persisted state row, added or dismissed (migration 036). The caller loads
  // them and passes them here; this module reads no database of its own.
  //
  // This is the record, where `captured` above is only an inference drawn from
  // what the register happens to say right now. It is the stronger of the two:
  // an answered candidate stays answered whatever its captured text is later
  // edited to, which is the add-then-edit case the record exists for.
  actedCandidateIds = null,
} = {}) {
  ...
  return {
    ...
    captured: captured ?? {},
    // Normalised to a Set of non-empty keys, so a caller may pass a Set, an
    // array, or nothing at all. Absent reads as an empty set, which suppresses
    // nothing and leaves the behaviour exactly as it was before this existed.
    actedCandidateIds: toKeySet(actedCandidateIds),
  };
}

// A Set of candidate keys from a Set, an array, or null.
...
/**
 * The comparison form for the already-captured check: trimmed, inner whitespace
 * collapsed, lowercased.
 *
 * This is EXACT matching, not fuzzy matching. It normalises the ways the same
 * string can be written (a trailing space, a line break, a capital), and nothing
 * else: no edit distance, no synonyms, no stemming, no similarity threshold.
 * Deciding that "Off-plan presales absorption slower than planned" and "Off-plan
 * sales absorption runs slower than the drawdown the programme assumes" are the
 * same item is a JUDGEMENT, and this engine does not make judgements. Those two
 * are both offered, and the developer dismisses the one they do not want.
 */
 ...
 *
 *   suppressed  [{ key, reason }] for every candidate whose condition held and
 *               which the developer has already dealt with. Two reasons land in
 *               this one bucket, and a candidate caught by both appears once:
 *
 *                 already_acted     it carries a persisted state row, added or
 *                                   dismissed (migration 036). The RECORD of
 *                                   what the developer did, checked first and
 *                                   winning outright: an answered candidate
 *                                   stays answered whatever its captured text
 *                                   now says.
 *                 already_captured  its rendered text is ALREADY in the
 *                                   project's list of that type. An INFERENCE
 *                                   from the register as it stands, which still
 *                                   covers a project that captured an item
 *                                   before any state row existed for it. Exact
 *                                   matching only, see normaliseText.
 *
...
export function deriveRaidSuggestions(baseline, library = RAID_LIBRARY) {
  ...
  // The already-captured index, one normalised set per RAID type.
  const capturedByType = {};
  for (const type of RAID_TYPE_ORDER) {
    capturedByType[type] = new Set(
      (base.captured?.[type] ?? []).map(normaliseText).filter((t) => t !== '')
    );
  }
  ...
  for (const candidate of library ?? []) {
    ...
    // Already answered on this project, so it is not offered again whatever the
    // register now says. Checked BEFORE the render, because the record of the
    // developer's decision does not depend on the wording: a candidate that
    // could no longer be worded from the baseline is still one they answered.
    if (base.actedCandidateIds.has(candidate.key)) {
      suppressed.push({ key: candidate.key, reason: 'already_acted' });
      continue;
    }
    ...
    // Already in the list, so suggesting it again is noise. Checked after the
    // render, because what the developer would compare against is the rendered
    // text, not the template.
    if (capturedByType[candidate.type]?.has(normaliseText(text))) {
      suppressed.push({ key: candidate.key, reason: 'already_captured' });
      continue;
    }
  ...
  return { candidates, dropped, suppressed };
}
...
/**
 * How many candidates each RAID type is hiding, so the band can say so rather
 * than just being shorter than it was. Every type gets a key, for the same
 * reason groupByType gives every type one.
 *
 * The suppressed entries carry the candidate key, not its type, so the type is
 * read back from the library the derivation ran against. A key the library does
 * not hold counts towards nothing, which is what a retired candidate should do.
 */
export function suppressedCountByType(suppressed, library = RAID_LIBRARY) {
  const typeByKey = new Map((library ?? []).map((c) => [c.key, c.type]));
  const counts = {};
  for (const type of RAID_TYPE_ORDER) counts[type] = 0;
  for (const entry of suppressed ?? []) {
    const type = typeByKey.get(entry?.key);
    if (type != null && counts[type] !== undefined) counts[type] += 1;
  }
  return counts;
}
