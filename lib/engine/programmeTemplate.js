 ...
 * extended to two levels by Programme module Phase 1 Foundation; drill-down
 * milestones added by the Programme module drill-down milestones step). A
 * versioned, region-neutral schedule skeleton for a property development: the
 * eight Flitrr Framework lifecycle stages (0 to 7), each now an ordered list of
 * activities with a typical duration, the key milestones re-homed onto the
 * activity each sits under, a typical gate duration in weeks, and any
 * location-sensitive checkpoints.
 *
 ...
 *   - Each milestone carries a `tier`, headline or drill-down (see MILESTONE_TIER).
 *     A headline milestone is developer-facing: it is shown in the Brief wizard and
 *     the Brief, dated by the developer, and reality-checked at set-up. A
 *     drill-down milestone is template-derived and never set by the developer: it
 *     gives a mid-stage activity a completion point so progress reads in real steps
 *     under the percent-complete rule, but it is never surfaced to the developer
 *     and never reality-checked (specification Section 7). stageMilestones returns
 *     the headline tier only; the assembly engine reads the drill-down milestones
 *     straight off each activity and places them on the agreed skeleton. The nine
 *     milestones the one-level template held are headline; the four mid-stage
 *     completion milestones added on activities 1a, 2a, 3a and 5a are drill-down.
...
// The two milestone tiers, by where the milestone surfaces. A headline milestone
// is developer-facing: shown in the Brief wizard and the Brief, dated by the
// developer, and reality-checked at set-up. A drill-down milestone is
// template-derived and never set by the developer: it gives a mid-stage activity
// a completion point so progress reads in real steps, but it is not surfaced to
// the developer and not reality-checked (specification Section 7). stageMilestones
// returns the headline tier only; the assembly engine reads the drill-down
// milestones straight off each activity. The nine milestones the one-level
// template held are headline; the four mid-stage completion milestones are
// drill-down. Frozen so a consumer cannot mutate the vocabulary.
export const MILESTONE_TIER = Object.freeze({
  HEADLINE: 'headline',
  DRILLDOWN: 'drilldown',
});
...
// against. The two-level activities shape lands at 1.1.0; the four drill-down
// milestones (on activities 1a, 2a, 3a and 5a) land at 1.2.0.
export const PROGRAMME_TEMPLATE_VERSION = '1.2.0';
        ...
        activity('1a_brief_feasibility', 'Brief and feasibility', 3, [
          {
            ...
            name: 'Brief and feasibility confirmed',
            serves: SERVED_OBJECTIVES.SCOPE,
            offsetWeeks: 3,
    ...
    },
    {
      ...
      activities: [
        activity('2a_scope_selection', 'Scope and selection', 4, [
          {
            key: 'consultant_scope_agreed',
            name: 'Consultant scope agreed',
            serves: SERVED_OBJECTIVES.SCOPE,
            offsetWeeks: 4,
            tier: MILESTONE_TIER.DRILLDOWN,
          },
    ...
    },
    {
      ...
      activities: [
        activity('3a_design_development', 'Design development', 8, [
          {
            key: 'developed_design_complete',
            name: 'Developed design complete',
            serves: SERVED_OBJECTIVES.QUALITY,
            offsetWeeks: 8,
            tier: MILESTONE_TIER.DRILLDOWN,
          },
...
/**
 * The flat, ordered HEADLINE milestone list for a stage, walked out of its
 * activities. The two-level template homes milestones under activities, but the
 * developer-facing derivations (the advised dates, the Brief and wizard milestone
 * view, and the reality check) read a stage's milestones as one flat list in
 * order, so this is the single place that flattens them: activity order, then
 * milestone order within each activity, which preserves the order the one-level
 * template held.
 *
 * It returns the HEADLINE milestones only. Drill-down milestones (those tagged
 * MILESTONE_TIER.DRILLDOWN) are template-derived completion points that are never
 * surfaced to the developer and never reality-checked (specification Section 7),
 * so they are filtered out here, and every developer-facing derivation that reads
 * a stage's milestones through this one helper stays headline-only with no change
 * of its own. The assembly engine does NOT read through here: it reads the
 * drill-down milestones straight off each activity's `milestones` and places them
 * on the agreed skeleton, which is how the four drill-down milestones reach v1.
 *
 * Accepts a stage in either shape. A two-level stage ({ activities }) is walked;
 * a legacy or foreign one-level stage ({ milestones }) is read as is, so the "or
 * any object with the same shape" contract the derivations document still holds.
 * An untagged milestone (a legacy or foreign one with no tier) is treated as
 * headline, so the filter only ever drops an explicitly tagged drill-down.
 * Returns a new array; never mutates the stage.
 */
export function stageMilestones(stage) {
  const all = Array.isArray(stage?.activities)
    ? stage.activities.flatMap((a) => a?.milestones ?? [])
    : (stage?.milestones ?? []);
  return all.filter((m) => m?.tier !== MILESTONE_TIER.DRILLDOWN);
}
...
/**
 * The summed within-norm band for a stage, in whole weeks: the total of its
 * activities' within-norm minima and maxima. It is the generous band a gate's
 * implied stage span is checked against in the Phase 1.1 reality check, because a
 * gate bounds the whole stage. Its centre is the sum of the activity typicals
 * (stageActivityWeeks), so a span at the centre is the typical total. It reads the
 * frozen activity bands; it does not re-derive the band rule (withinNormBand).
 * Pure; sits beside stageActivityWeeks so the stage-band derivations live in one
 * place. Returns { min, max }; both zero for a stage with no activities.
 */
export function summedActivityBand(stage) {
  return (stage?.activities ?? []).reduce(
    (acc, a) => {
      const band = a?.withinNormWeeks ?? { min: 0, max: 0 };
      return { min: acc.min + (band.min ?? 0), max: acc.max + (band.max ?? 0) };
    },
    { min: 0, max: 0 }
  );
}
