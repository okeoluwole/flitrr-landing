...
import { monthlyCashflow } from './cashflow.js';

// The engine version, stamped onto every saved scheme (stack_schemes, 028) so
// a stored input set records what computed it. Results are never stored; a
// loaded scheme recomputes, and this stamp is how a recompute under a later
// engine is distinguishable from the run that was saved. Bump it on any change
// that alters a computed figure; presentation-only changes leave it alone.
...
export function computeAppraisal(inputs) {
  ...
  const cashflow = monthlyCashflow(inputs, switches, funding);
