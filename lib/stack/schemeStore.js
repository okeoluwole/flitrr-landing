...
// to fetch however large each input set is. The embedded projects(name) is the
// spine attachment (039): the linked project's name rides along so the list
// can say what a scheme appraises without a second read.
export const SCHEME_SUMMARY_COLUMNS =
  'id, name, engine_version, created_at, updated_at, project_id, projects(name)';
...
export function schemeSummary(row) {
  return {
    ...
    // The spine attachment (039). Both null for an unlinked scheme, and for
    // every scheme saved before the attachment existed.
...
/**
 * Save a new scheme: a name, the complete engine input set, the engine
 * version stamping what computed it, and optionally the project it appraises
 * (the 039 spine attachment; null saves an unlinked scheme). organisation_id
 * and created_by are the database's to fill (the 028 trigger and the caller's
 * session), and the 039 trigger refuses a project outside the organisation.
 * A blank name is rejected before the write. Returns { scheme, error }, the
 * stored summary row on success.
 */
export async function insertScheme(
  supabase,
  { name, inputs, engineVersion, projectId = null }
) {
...
/**
 * Save over an existing scheme: replace its name, inputs, engine version
 * stamp, and project link in one write. The link is set absolutely, so a
 * save-over with projectId null UNLINKS the scheme rather than preserving a
 * stale link. created_by and created_at are untouched (the audit stamp
...
export async function updateScheme(
  supabase,
  { id, name, inputs, engineVersion, projectId = null }
) {
  const cleanName = cleanSchemeName(name);
  if (cleanName === null) {
    return { scheme: null, error: new Error('a scheme name is required') };
