/**
 * Risk monitoring model (M6.2.1). Pure, deterministic monitoring logic shared
 * by the Risk module and, later, Programme and Dashboard. No DB, no React, no
 * network, no system clock: every input is passed in, so the same inputs always
 * give the same verdict and the whole module is unit-testable in isolation.
 *
 * It sits beside riskModel.js and REUSES its severity derivation
 * (deriveSeverity) and severity ranking (SEVERITY_RANK) rather than duplicating
 * them, so the register, the Brief, and monitoring all speak one severity
 * vocabulary. The explicit .js on that import keeps this module runnable under
 * Node (for the truth-table check) as well as under the Next bundler.
 *
...
import { deriveSeverity, SEVERITY_RANK } from './riskModel.js';
 ...
 *                       severity key (see riskModel SEVERITY_RANK). 'serious'
 *                       means Serious only; 'moderate' means Worth watching and
 *                       above.
...
/**
 * D3 / D4: a risk's LIVE criticality, derived from the objective it threatens.
 * Distinct from the stored project_risks.criticality snapshot, which this
 * module never reads. Returns one of:
 *   'critical'  the linked objective is non-negotiable
 *   'standard'  the linked objective is flexible (any non non-negotiable value)
 *   'unlinked'  no linked objective (D4): "needs a link", never a silent standard
 *
 * objectivesById: a map of objective id -> { classification, ... } (the live
 * project_objectives rows), supplied by the caller.
 */
export const EFFECTIVE_CRITICALITY = {
  CRITICAL: 'critical',
  STANDARD: 'standard',
  UNLINKED: 'unlinked',
};
...
export function effectiveCriticality(risk, objectivesById) {
  const objectiveId = risk?.linked_objective_id;
  const objective = objectiveId ? objectivesById?.[objectiveId] : null;
  if (!objective) return EFFECTIVE_CRITICALITY.UNLINKED;
  return objective.classification === 'non_negotiable'
    ? EFFECTIVE_CRITICALITY.CRITICAL
    : EFFECTIVE_CRITICALITY.STANDARD;
}
...
// Ordering rank by live criticality: critical first, then unlinked (a
// governance gap that can hide a critical risk, so it sits above standard),
// then standard. These rank the attention list; they are not thresholds. Final
// presentation order is M6.2.3's call.
const CRITICALITY_RANK = {
  [EFFECTIVE_CRITICALITY.CRITICAL]: 0,
  [EFFECTIVE_CRITICALITY.UNLINKED]: 1,
  [EFFECTIVE_CRITICALITY.STANDARD]: 2,
};
...
function firesCriticalUnmanaged(effective, risk) {
  return (
    effective === EFFECTIVE_CRITICALITY.CRITICAL &&
...
export function assessRisk(risk, objectivesById, now) {
  ...
  const needsLink = effective === EFFECTIVE_CRITICALITY.UNLINKED;
  const firedTriggers = [];
