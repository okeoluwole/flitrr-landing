...
/**
 * The Programme Schedule tab display model (Phase 3.7, restructured for Note
 * 17 finding 3). Proves the one register derives correctly from what the page
 * already holds: the row set joins the frozen baseline, the forecast tree,
 * and the RAG flags without touching any of them; the four columns read Item
 * off the baseline, Baseline off the baseline, Current off the forecast
 * engine, and Variance as the display subtraction with its direction plain
 * across ahead, on baseline, and behind; the register's filter keeps exactly
 * the gates, the critical milestones, and anything flagged in the governing
 * view and every point in the other, in programme order either way; the
 * flagged part follows the RAG derivation the tolerance dial re-runs; the
 * grouping is by stage in programme order; and no helper mutates an engine
 * output. The chart over the same row set is chartModel.test.js.
 *
...
describe('filterRows', () => {
  it('governing keeps exactly the gates, the critical milestones, and anything flagged; a standard unflagged point is one toggle away', () => {
    const { rows } = derive();
    const governing = filterRows(rows, 'governing');

    // m7 is standard but flagged, so the flag pulls it up; m2 and m3 are
    // standard and unflagged, so they appear only under All points.
    expect(new Set(governing.map((r) => r.key))).toEqual(
      new Set(['m1', 'm4', 'gate_0', 'gate_1', 'gate_7', 'm7'])
    );

    const all = filterRows(rows, 'all');
    expect(all.map((r) => r.key)).toEqual(rows.map((r) => r.key));
    expect(all.some((r) => r.key === 'm2')).toBe(true);
    expect(all.some((r) => r.key === 'm3')).toBe(true);
  });

  it('keeps programme order in both views and never reorders the given rows', () => {
    const { rows } = derive();
    const order = rows.map((r) => r.key);

    // Stage 7's points sit early in time, but the register reads in programme
    // order, so no view pulls them ahead of stage 0's gate.
    expect(filterRows(rows, 'governing').map((r) => r.key)).toEqual([
      'm1',
      'gate_0',
      'm4',
      'gate_1',
  ...
  it('an unknown or missing key falls back to the default view, so the register always has rows', () => {
    const { rows } = derive();
    const fallback = filterRows(rows, 'not-a-view').map((r) => r.key);
    expect(fallback).toEqual(
      filterRows(rows, DEFAULT_REGISTER_FILTER).map((r) => r.key)
    );
    expect(filterRows(rows).map((r) => r.key)).toEqual(fallback);
    expect(REGISTER_FILTERS.map((f) => f.key)).toEqual(['governing', 'all']);
  ...
  it('follows the RAG derivation the tolerance dial re-runs: membership tracks the flagged list, and the carried colour tightens with the tolerance', () => {
    ...
    expect(
      filterRows(rowsFlagged, 'governing').some((r) => r.key === 'm7')
    ).toBe(true);
    expect(
      filterRows(rowsUnflagged, 'governing').some((r) => r.key === 'm7')
    ).toBe(false);

    // On the live observed-slip rule the tolerance moves a critical point's
    // colour, and the register carries the fresh colour through: m1, three
    ...
    const m1Standard = filterRows(standard.rows, 'governing').find(
      (r) => r.key === 'm1'
    );
    const m1Tight = filterRows(tight.rows, 'governing').find(
      (r) => r.key === 'm1'
    );
  ...
  it('the filtered set groups by stage with no empty stage left behind', () => {
    const { rows } = derive();
    const groups = registerGroups(filterRows(rows, 'governing'));
    expect(groups.map((g) => g.stage)).toEqual([0, 1, 7]);
    expect(groups[0].rows.map((r) => r.key)).toEqual(['m1', 'gate_0']);

    // A stage whose whole content the filter drops raises no group at all,
    // so the register never shows an empty stage heading.
    const standardOnly = rows.filter((r) => r.key === 'm2' || r.key === 'm4');
    expect(
      registerGroups(filterRows(standardOnly, 'governing')).map((g) => g.stage)
    ).toEqual([1]);
  });
});
...
describe('nothing is mutated', () => {
  it('no helper writes to the baseline, the engine outputs, or the given rows', () => {
    ...
    registerGroups(filterRows(rows, 'governing'));
    registerGroups(filterRows(rows, 'all'));
